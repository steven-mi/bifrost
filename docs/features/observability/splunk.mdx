...
- **Cost-aware**: content logging is off by default (Splunk is priced on ingest volume); enable it explicitly when you need prompt/response bodies.
...
Events go to `POST <endpoint>/services/collector/event`; metrics go to `POST <endpoint>/services/collector` as `metric`-typed HEC events. Delivery is fire-and-forget: if Splunk is slow or unreachable, packets are dropped rather than stalling requests.
...
3. **Scope the token** to allow both indexes (or leave it unrestricted).
...
| `insecure_skip_verify` | `boolean` | ❌ | `false` | Disable TLS certificate verification. Use only for on-prem HEC with a self-signed cert. See [TLS](#tls). |
| `custom_fields` | `object` | ❌ | - | Static key/value fields added to every event and metric. |
| `request_headers` | `string[]` | ❌ | - | Request-header name patterns to capture onto events. Supports exact names and wildcards (`x-custom-*`, `*`). |
...
- **`insecure_skip_verify`**: disable verification entirely. This encrypts the connection but does not authenticate it, leaving it open to man-in-the-middle attacks.
...
It also does **not** cover attribution identifiers, which remain on events and metrics. Values captured via `request_headers` are attached regardless of this flag, so only enable header capture for headers you intend to export.
</Warning>
